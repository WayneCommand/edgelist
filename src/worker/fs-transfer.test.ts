...
function adapter(
	...
	capabilities: string[] = ["read", "copy", "move", "merge"],
) {
...
function dependencies(
	mounts: Array<ReturnType<typeof adapter>>,
	mountPaths = mounts.map((_, index) => (index === 0 ? "/" : `/mount-${index}`)),
	options: { virtual?: string[]; childMounts?: Record<string, string[]> } = {},
): TransferPlannerDependencies {
	const configs = mountPaths.map((mountPath) => config(mountPath));
	const storageFor = (path: string) =>
		[...mounts]
			.map((mount, index) => ({ mount, mountPath: configs[index].mount_path }))
			.sort((left, right) => right.mountPath.length - left.mountPath.length)
			.find(
				(item) =>
					item.mountPath === "/" ||
					normalizePath(path) === item.mountPath ||
					normalizePath(path).startsWith(`${item.mountPath}/`),
			)!;
	return {
		resolve: async (path) => {
			...
			return {
				...
				path:
					selected.mountPath === "/"
						? normalizePath(path)
						: normalizePath(path).slice(selected.mountPath.length) || "/",
				adapter: selected.mount.adapter,
			};
		},
		isVirtualMount: async (path) => Boolean(options.virtual?.includes(normalizePath(path))),
		listVirtualMounts: async (path) =>
			(options.childMounts?.[normalizePath(path)] ?? []).map((name) => ({ ...object(name, true), name })),
	};
}
...
describe("synchronous same-storage transfers", () => {
	it("copies a file into another directory", async () => {
		...
		const result = await planTransfers(
			"copy",
			{ src_dir: "/", dst_dir: "/directory", names: ["file.txt"] },
			dependencies([root]),
		);
		expect(result).toMatchObject({ operation: "copy", accepted: 1, skipped: 0, failed: 0 });
		expect(first(root)).toEqual({
			source: "/file.txt",
			destination: "/directory/file.txt",
	...
	it("supports overwrite, skip, and directory merge policies", async () => {
		...
		await expect(
			planTransfers("copy", { src_dir: "/", dst_dir: "/directory", names: ["file.txt"], skip_existing: true }, deps),
		).resolves.toMatchObject({ skipped: 1 });
		await expect(
			planTransfers("copy", { src_dir: "/", dst_dir: "/directory", names: ["file.txt"], overwrite: true }, deps),
		).resolves.toMatchObject({ accepted: 1 });
		expect(first(root)?.options.overwrite).toBe(true);

		const merge = await planTransfers(
			"copy",
			{ src_dir: "/", dst_dir: "/target-dir", names: ["source-dir"], merge: true },
			deps,
		);
	...
	it("rejects cross-storage and mount-boundary targets", async () => {
		...
		const two = adapter(
			"/nested",
			new Map([
				["/nested/directory", object("/nested/directory", true)],
				["/nested/file.txt", object("/nested/file.txt")],
			]),
		);
		const deps = dependencies([one, two], ["/", "/nested"], { virtual: ["/group"] });
		const cross = await planTransfers(
			"copy",
			{ src_dir: "/", dst_dir: "/nested/directory", names: ["file.txt"] },
			deps,
		);
		...
		await expect(planTransfers("copy", { src_dir: "/", dst_dir: "/group", names: ["file.txt"] }, deps)).rejects.toThrow(
			"Destination is a virtual mount directory",
		);
	});
	...
	it("prevents storage roots, self-nesting, and directories containing mounts", async () => {
		...
		expect(withMount.results[0].error).toBe(
		...
		const storageRoot = await planTransfers(
			"move",
			{ src_dir: "/", dst_dir: "/nested/directory", names: ["nested"] },
			nestedDeps,
		);
