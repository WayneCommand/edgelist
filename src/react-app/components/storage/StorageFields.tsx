...
export function StorageFields({ drivers, draft, onChange, onOpenJson }: StorageFieldsProps) {
	...
	if (!info) return <p className="py-6 text-center text-sm text-muted">Loading drivers…</p>;
	...
	async function changeDriver(next: string) {
		...
		if (draft.id > 0 && Object.keys(addition).length > 0) {
			const ok = await confirm({
				title: "Switch driver",
				message: `Switching to ${target?.name ?? next} clears the current driver settings.`,
				confirmLabel: "Switch",
				danger: true,
			});
			if (!ok) return;
	...
	return (
		<>
			<section>
				...
				<div className="grid gap-3 sm:grid-cols-2">
					<label className="block text-sm">
						<span className="mb-1.5 block font-medium text-foreground">Driver</span>
						<select
							value={draft.driver}
							aria-label="Driver"
							className="w-full rounded-lg border border-border bg-field-background px-3 py-2 text-sm text-foreground outline-none transition focus:border-focus focus:ring-4 focus:ring-accent/15"
			...
			<section>
				<h3 className="mb-3 text-base font-semibold">{info.name} settings</h3>
				<div className="grid gap-3 sm:grid-cols-2">
