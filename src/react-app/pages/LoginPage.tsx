		...
		<main className="flex min-h-screen items-center justify-center bg-background p-6 text-foreground">
			<HeroCard className="w-full max-w-md" variant="default">
				<div className="mb-8 text-center">
					...
					<h1 className="text-2xl font-semibold tracking-tight">Sign in to EdgeList</h1>
					<p className="mt-2 text-sm text-muted">OpenList-compatible file management</p>
				</div>
				<form className="space-y-5" onSubmit={submit}>
					<label className="block">
						<span className="mb-2 block text-sm font-medium text-foreground">Access Key</span>
						<input
							required
							value={accessKey}
					...
					<label className="block">
						<span className="mb-2 block text-sm font-medium text-foreground">Secret Key</span>
						<input
							required
							type="password"
							value={secretKey}
				...
				<p className="mt-8 text-center text-xs text-muted">Credentials are verified securely by the Worker.</p>
			</HeroCard>
		</main>
	);
}
