...
2. 无浏览器，**所有动效只做了代码级审查，没有在 Animations 面板按 10% 速度回放**。UI-1 已在此前提下提交（按压缩放只动了 `scale` 与过渡归属，风险已降到最低）。**UI-8 / UI-8 补 / UI-9 是在同一前提下提交的，本文件不声称它们通过了浏览器验收**——这一点必须如实记下，因为原条目把这一步写成了 UI-8 的前置条件，而它并没有被完成。动效与主题的改动**在浏览器里仍需逐条确认**：
   - 逐速回放：`SelectionBar` 首次选中的入场（300ms）与退场（150ms）、`pb-24` 是否与浮条同步、拖放浮层的淡入淡出、错误横幅与空态的两拍、右键菜单的入场 150ms / 退场 100ms（含退场**不会弹回全不透明**）。
   - `prefers-reduced-motion: reduce` 下：以上全部无动画，但**状态仍然可见**（尤其菜单必须仍能关闭——卸载用的是 `setTimeout` 而非 `animationend`，正是为了这个）。
   - 主题切换：切换瞬间**整页无过渡**（四步顺序若被破坏会立刻表现为全页颜色渐变）；`system` 档下改系统外观应跟着变；刷新时**不应出现白底闪一下**（`index.html` 内联脚本的意义）；`currentTheme()` 的既有缺口——**已打开的 Monaco 编辑器会停留在旧主题**，直到下次打开文件才切换（`MonacoTextEditor` 读一次主题即可，见其注释；若要修需以 key 强制重挂，会丢掉未保存的编辑，故未做）。
   - 暗色下的对比度仍未实测（见第 3 条）。
   3. `--accent` 与 `--*-soft` 的对比度未测量（`accent-soft` 上的选中文字、`disabled:opacity-40` 叠在 `text-muted` 上）。UI-5 会把分段控件的选中态切到 `--accent-soft-foreground`，届时一并实测。**UI-9 之后这条的范围扩大了**：暗色皮肤从"永远不可达"变成默认可达（`system` 档在暗色系统上就是暗色），所以**所有 token 需要在暗色下重测一遍**，而此前不必。UI-10 已把三处 `bg-accent-soft + text-accent` 改成 `text-accent-soft-foreground`（提高了对比度），但两个方向都还没量过。
...
6. **本机 `pnpm` 不可用（离线）**，全部命令用 `node_modules/.bin/` 下的本地二进制替代（`prettier --check src`、`eslint .`、`tsc -b`、`vitest run --config vitest.config.ts`、`vite build`、`wrangler deploy --dry-run`）。另：环境里 `/tmp` 是 10MB 的 tmpfs，`vitest` 会以 `ENOSPC` 失败，需先 `export TMPDIR=/home/wayne/.cache/wb-tmp`。`package.json` 的 `lint` 实为 `eslint . && prettier --check src`，**不含 `.md`**——所以本文件的格式不在校验范围内。
...
| UI-5 | ✅ | 采纳用户决策「index.css 一条全局配方」。`index.css` 的输入框规则重写为**一条 `:where(input:not([type=file],[type=checkbox],[type=radio]), select, textarea)` 配方**，取值全部对齐库的 `.input`：`--field-background` / `1px solid var(--border)` / `var(--field-radius)` / `var(--field-shadow)` / `--field-foreground`，hover 用库的 `--field-hover` + `--field-border-hover`，focus 从「20% 不透明度的 2px outline」改为**实心 2px accent ring**（`box-shadow: var(--field-shadow), 0 0 0 2px var(--focus)`），补 `:disabled`（`--cursor-disabled` / `--disabled-opacity`）与 `prefers-reduced-motion`；排除 checkbox/radio 是必须的（HeroUI `Switch` 经 react-aria 渲染的就是 `<input type="checkbox">`）。12 处字段类名只剩布局（`LoginPage` 的 `transition`、`DriverField`/`StorageJsonDialog` 的 `transition-[border-color,box-shadow]`、各处的 `outline-none`/`focus:border-focus`/`focus:ring-4` 全部删除）。项目的 `--field-radius` / `--field-border` 重复声明删除（库的 `variables.css:35/76` 已有，值一致）。`TransferDialog` 的取景框 `bg-field-background`→`bg-field` + `rounded-field`。构建产物确认配方与嵌套的 `::placeholder` / `:hover` / `:focus` / `:disabled` / `@media` 全部正确生成，且输出里**再无** `bg-field-background`。commit `3400dcf` |
| UI-6 | ✅ | `StorageTableSkeleton` 重写为**逐列镜像真实表头**（原来只有 3 列对 6 列，数据落位时表头标签随行一起出现、把内容往下推），断点也一并镜像，并自己画出表头带；`MetadataPage` 的「Loading」文字换成 `MetadataListSkeleton`（行的形状对齐一条规则 + 右侧两个动作按钮，按钮用 HeroUI `sm` 的真实尺寸 `h-9 md:h-8`）；新增 `components/common/EmptyState.tsx`（`size-12 rounded-full bg-surface-secondary` 圆盘 + `InboxIcon` + `text-sm text-muted` 文案），`FilesPage`、`StoragesPage`、`MetadataPage` 三处空态改用它——改之前三处分别是「居中 + 64px 内边距」和两处「左对齐 + 32px」，不一致。新增 `common-views.test.tsx`（2 条）并扩充 `storage-views.test.tsx`（+3）。commit `a90521c` |
| UI-7 | ✅ | 新增 `components/files/preview/metrics.ts` 收口几何：`PANEL`（`h-[min(68vh,640px)] min-h-[360px]`）、`FRAME`、`WELL`、`PAGE`、`PAD`。此前图片 240px 下限、PDF 固定 640、编辑器第三个数、markdown 第五个数，**同一弹窗翻到下一个文件就换一个尺寸**；`ImageViewer` / `PdfViewer` / `MediaViewer` / `UnsupportedViewer` / `MarkdownViewer` / `index.tsx` 全部改为组合常量。新增 `--image-outline` 双 token（亮 `oklch(0 0 0/.1)`、暗 `oklch(1 0 0/.1)`，**纯黑/纯白 10%，不用带色调的中性色**——后者会吸周围面色、沿边看起来像脏）与 `@utility image-outline`（`outline` + `outline-offset: -1px`，画在边内所以不改图片尺寸），只给 `<img>`。新增 `lib/theme.ts`（`currentTheme()` 同时认 `[data-theme="dark"]` 与 `.dark`，与 `index.css` 的两个选择器对齐），`MonacoTextEditor` 从写死 `Default Dark Modern` + `bg-[#1e1e1e]`（亮色界面里唯一一块黑板）改为按主题选 VS Code 自带的两套主题、外框改用 `PANEL`+`FRAME`。**写测试时发现并修掉一处自相矛盾**：空 markdown 的「This file is empty.」原先也套了 `PANEL`，与 `metrics.ts` 刚立的规则（一句话不该占 640px，同音频播放器与无预览器提示）冲突，去掉其高度。新增 `preview-views.test.tsx` 6 条几何断言 + `lib/theme.test.ts` 5 条；构建产物确认 `.image-outline`、两条 `--image-outline` 与面板高度规则均正确生成。commit `b784892` |
| UI-8 | ✅ | 三处"原地出现"的面改为**隐藏而非移除**——条件渲染的元素没有"从哪来"的帧，transition 无从谈起。`SelectionBar` 常驻并靠 `opacity` + `translate-y` 过渡（入场 300ms / 退场 150ms：退场只是确认用户自己的动作，入场才是新信息；**时长写在"要去"的那个状态上**，因为支配过渡的是目标态的 `transition-*`），配套 `FilesPage` 的 `pb-24` 一起过渡（否则一次操作列表跳两下）。**这里有一个必须处理的副作用**：常驻后浮条内的按钮仍在文档里，而面板又靠 `pointer-events-auto` 保持可点击，所以不加处理它会**盖住列表最后一行的点击**并被 Tab 命中——用 `inert`（React 19 原生支持，且已验证 React 在 `false` 时**整个属性不渲染**，因为布尔属性"存在即生效"）。拖放浮层同理常驻，但它只含不可聚焦的提示文字，用 `aria-hidden` 即可（`opacity` 不会把内容移出无障碍树）。错误横幅与空态**无法用 transition**（挂载即出现，没有起始态），两者共用**一个只写 `from` 的 `@keyframes arrive`**（终点留给元素自身，于是不必在样式表里重述设计），throw 与 beat 走两个变量；空态两拍（disc 先落、句子 100ms 后跟），错误只一拍——错误不是序列。`ContextMenu` 与 `Modal` **未动**：右键高频，加延迟即"卡"。新增 7 条断言（浮条隐藏/显示各 2、拖放浮层 2、空态 stagger 1；原"空选时不渲染"的断言按新设计作废并替换）。597 测试通过。**本步不含"空态加动作按钮"**：§1.11 的 `EmptyState`（图标 + 标题 + 说明 + 可选动作）只落地了前三项，动作按钮需要逐页决定"下一步该干嘛"（上传？返回上级？），属功能决策而非动效，**仍待办**。commit `1701f4f` |
| UI-8 补 | ✅ | 落地 **§6.2 第 4 条修正**（§1.8 原写"右键菜单保持不动"，读库后改为"采用库的 popover 动效"）。这个修正**必须补做**——§3 第 7 条（"菜单/弹窗不加动画"）是先写的、§6.2 是读库后的**修正**，二者冲突时以后者为准；且"全站浮层都动、只有菜单硬闪"比"都不动"更糟。配方是**照抄不是发明**：`animate-in` / `animate-out` / `zoom-in-90` / `fade-in-0` / `fill-mode-forwards` 全部来自库已依赖的 `tw-animate-css`（`@keyframes enter`/`exit` 由 `--tw-enter-*` / `--tw-exit-*` 驱动，`duration-*` 写进 `--tw-duration`），即库自己的 `.popover[data-entering]` 用的同一组类；`origin-top-left` 让缩放的锚点落在指针所在的角而非菜单中心。**退场需要一个"由谁负责延迟卸载"的答案**：父组件只管"菜单是否存在"、不管"是否还在离场"，所以由菜单自己标记 `exiting`、100ms 后再调 `onClose`；配 `fill-mode-forwards` 把淡出后的最后一帧钉住，否则退场动画播完会**弹回全不透明**再被卸载。`close()` 用 ref 做幂等（点菜单项的 click 会冒泡到背后的 backdrop，同一次意图会到达两次）。Escape 也走同一条 close 路径，所有退出方式观感一致。**卸载用 `setTimeout` 而非 `onAnimationEnd`**：为了满足 `prefers-reduced-motion` 必须加 `motion-reduce:animate-none`，而动画一旦被禁用就**没有 `animationend` 事件**，菜单将永远无法关闭——这是本步最容易埋的一个坑。新增 1 条断言 + 构建产物确认 5 个动画类全部生成。commit `adb7dff` |
| UI-9 | ✅ | 采纳用户决策「接一个真实开关」。暗色 token（整套 + `color-scheme: dark`）与 `.theme-transitioning` 规则**都已写好但永远不可达**（`<html>` 上没有 `data-theme` 也没有 `dark`），本步把它们接通。核心是**把"选择"与"主题"分开**：`ThemeChoice`（system/light/dark）是偏好，`Theme`（light/dark）是外观——把选择**解析后的结果**存下来会把 system 冻死（选了"跟随系统"的人会永远跟随选择那一刻的系统），所以存的是选择本身，并只在 `choice === "system"` 时监听 `matchMedia` 的 change；显式选择在两个方向上都压过系统。**换主题不许触发过渡**的四步顺序一步都不能换：加 `.theme-transitioning` → 改 `data-theme` → **强制回流**（`void root.offsetHeight`）→ 双 `requestAnimationFrame` 后移除。少了强制回流，浏览器在移除 guard 时手里还是旧颜色，于是**照旧全页过渡**——这正是该规则存在的原因，也是它此前从未被验证过的原因。首帧传 `suppressTransitions: false`（屏幕上没有可过渡的起点，加 guard 只是给未加载完的页面多一帧延迟）。**两处必须在 React 之外**：`main.tsx` 里 `initTheme()` 在 `createRoot` 之前跑（组件太晚，挡不住错颜色）；`index.html` 内联脚本再重复一次取值——浏览器在**任何模块被求值之前**就已经绘制，否则暗色用户每次加载都会看到白底闪一下。代价是 key 与前缀写了两遍，两处都有注释指向 `lib/theme.ts`。**额外验证**：HeroUI 自己在同一选择器下也带一套暗色主题（`--background: oklch(12% .005 285.823)`），已确认本仓的暗色块在样式表中**更靠后**、同级特异度下胜出（`oklch(12% .0015 253.83)` 生效），不会被库的默认值盖掉。`theme.test.ts` 由 5 条扩到 22 条：**store 是模块级状态，写它的用例一律 `vi.resetModules()` + 动态 import 取新副本**，否则用例之间会互相影响、通过与否取决于执行顺序；`requestAnimationFrame` 桩同步回调，避免测试去等一次真实绘制。618 测试通过。commit `249eecd` |
| UI-10 | ✅ | 收尾四项。**① 分段控件**抽成 `SegmentedControl`：三份拷贝（`FileToolbar` 视图、`Pager` 分页模式、`MarkdownViewer` 渲染/源码）**唯一漂移的地方恰好是最要紧的地方**——三处都用 `bg-accent-soft` + `text-accent`，而库专门提供 `--accent-soft-foreground` 给这个组合，且 `App.tsx` 导航已经在用它，等于同一种选中态两种画法、且分段版是**对比度更低**的那个（§6.2 已标出，本步修正）。同心几何**原样保留**（8 + `p-0.5`2 = 内 6 = `rounded-md`）。**刻意不给选中项加 `font-medium`**：半粗会把按钮撑宽一两像素，每次切换整排都会抖。**② LogoMark**：`App.tsx`（36px/8px）与 `LoginPage.tsx`（56px/16px）两处品牌块此前各写一遍、圆角随尺寸漂移；抽成组件后圆角取**瓦片尺寸的 1/4 并用百分比表达**（`rounded-[25%]`），一个值在所有尺寸（含将来新增的尺寸）都成立，而不是两个像素值靠人工同步——漂移就是这么来的。`aria-hidden`（两处都有文字标识，字母是装饰）。**③ z-index**：四个散落字面量 10/20/20/30，其中两个浮层**共用 20**、谁在上面由 DOM 顺序决胜；改为 `@theme` 里的一组具名层级（`--z-index-header/drop/bar/menu` 10/20/30/40），经 Tailwind 的 `--z-index-*` 命名空间生成 `z-header`/`z-drop`/`z-bar`/`z-menu`，已确认 4 个类和 4 个变量都在产物里。**④ `lang` 与 `<title>`/favicon**：`lib/locale.ts` 在模块加载时与每次 `setLocale` 时同步 `<html lang>`（`lang="en"` 包着中文会被读错、断行与字体回退也会错）；`<title>` 从脚手架的 `Vite + React + TS` 换成 `EdgeList`，favicon 换成同款标识——**用矩形条画 E 而不是 `<text>`**（favicon 不能指望字体存在），主色由 `--accent` 的 oklch 换算成 sRGB `#0485f7`（主题 token 无法跨进独立文件）。**§1.12 的预览高度常量在 UI-7 已完成**，本步复查确认全仓除 `metrics.ts` 与断言外无残留字面量。新增 11 条断言（分段控件 5、LogoMark 3、`lang` 3）。629 测试通过；`wrangler deploy --dry-run` 通过（Worker 仍能带着新资源构建）。commit `daaa5cb` |

**全部 10 步（含 1 项 §6.2 修正）已落地。** 唯一未关闭的验收缺口见 §5.2。
