...
### R1 几何：一个旋钮，外加控件一档（**下表是 UI-20 修订后的当前值**）

`index.css` 的 `--radius` = **`0.375rem`（6px）**；控件档 `--radius-control` = **`0.625rem`（10px）**；字段圆角**绑到控件档**（`--field-radius: var(--radius-control)`）。

库的 `@theme inline` 里 `--radius-xs..4xl = calc(var(--radius) × 0.25/0.5/0.75/1/1.5/2/3/4)`，Tailwind 的 `rounded-*` 工具读的正是这一串。所以一个值同时落定全仓几何：

| token | 原始 | UI-11 | **现在** | 落点 |
| --- | --- | --- | --- | --- |
| `--radius-xs` | 2px | 1.25px | 1.5px | 未直接使用；守卫的容差基准 |
| `--radius-sm` | 4px | 2.5px | 3px | 骨架碎块 |
| `--radius-md` | 6px | 3.75px | 4.5px | 排序箭头、存储行动作、表头小盒子 |
| `--radius-lg` | 8px | 5px | 6px | 分段控件外层、预览框 |
| `--radius-xl` | 12px | 7.5px | 9px | Switch 轨道 |
| `--radius-2xl` | 16px | 10px | 12px | 菜单项、浮条动作 |
| `--radius-3xl`（`.card` / `.popover` / `.menu` 用） | 24px | 15px | **18px** | 面板、弹窗、浮条 |
| `--radius-4xl` | 32px | 20px | 24px | （未使用） |
| `--radius-control`（库无此档） | — | 8px | **10px** | 所有按钮、Pager、存储行动作 |
| `--radius-field` | 12px | 7.5px | **10px** | 所有输入框、下拉 |

**为什么面板是 18px：看外边那个数。** macOS 26 Tahoe 把窗口圆角推到 **26px**，macOS 27 Golden Gate 又收回 **15-16px**（`NSConvolutionOverride1`：10 = macOS 15、16 = Golden Gate、26 = Tahoe）。UI-11 定的 15px 因此**本来就正好落在 macOS 27 的窗口值上**——面板那一档当时是对的，真正偏方的是控件那一档（8px 按钮 / 7.5px 输入框）。所以 UI-20 只把旋钮抬一级到 18px（比窗口软一档，同一风格族），把主要幅度给控件：按钮与输入框 8/7.5 → **10px**，两者现在是同一个值。

**例外只有按钮一条，且它必须无层。** 库的 `.button` 走 `--radius-3xl`，18px 放在 36px 高的按钮上仍是药丸：

```css
@theme { --radius-control: 0.625rem; }   /* 10px，控件专用的一档 */
.button { border-radius: var(--radius-control); }
```

**切库已有的 token 不能写在 `@theme` 里。** `@theme` 的输出落在 Tailwind 的 `theme` 层，而库的 `--field-radius` 也在这一层、位置更靠后（`@import "@heroui/styles"` 展开在 Tailwind 自己的 theme 块之后）→ 同层同特异度，后者胜，写在 `@theme` 里等于没写。所以字段那条绑定放在文件尾部的**无层** `:root` 里。**这是量出来的**，见 §6.7 第 28 条。库的 `--radius` 反过来输给本仓，原因正是它分层而本文件不分层。
...
1. **同心圆角逐处复核**。全仓 30+ 处圆角一起变小，配错的地方会"看起来只是有点怪"，不会报错。**规则在 UI-12 被改写两次，最终版**（第一版"内层 = 外层 − 内边距"是等式，第二版我写成不等式并声称"偏大会切进外圈的弧"——**后者是错的，见 §6.5 第 12 条**；用参数方程算过：两个方向都不会穿透，内层的弧恒在外层弧内）：
   ...
   **UI-20 在旋钮 5px → 6px 之后复跑了一遍**：三对全部回到**严格同心**（间隙 0），代价是两处 class 必须跟着旋钮走——菜单与浮条的 inset 从 `p-1`(4px) 提到 `p-1.5`(6px)。因为面板对**只有在 `内边距 = --radius` 时间隙才为 0**（`外层 − 内边距 − 内层 = 3r − b − 2r = r − b`）；inset 不跟着旋钮走，就会把旋钮动的量原样加到角带上（§5.10 里「把菜单 inset 改回 `p-1`」这条探针报的就是它）。另外补上了一个一直漏掉的项：**分段控件外层有 1px 描边**，它画在盒子内侧、和 padding 一样吃掉角上那一像素，此前那笔账里没有它。
...
6. **视口回归**：1440×900、1280×800、1024×768（`sm` 与 `lg` 两个断点各自两侧）、**390×844（硬判据：四个目的地必须可达——今天在这一档整条消失）**。重点看：52px 顶栏折成两行后内容首行的位置、`SelectionBar` 的 `fixed bottom-4` 在文档流滚动下是否仍不遮住最后一行、`FilesPage` 的 `pb-24` 过渡是否仍需要（它当初就是为 `SelectionBar` 留的）。
   **UI-13 做掉了一半，另一半仍然欠着**：代码侧的修复已完成——`nav` 上的 `hidden sm:flex` 没了，`<sm` 折两行，`order-last` 让 Tab 顺序在两种宽度下都是 品牌 → 导航 → 控件 → 内容。而且**这个错误可被静态标记看见**，所以补了 `app-shell.test.tsx` 守着它（`hidden` 一旦回来第 1 条就红，探针验过）。**仍然要浏览器的是纯像素那一半**：390px 下第二行是否真的折下来、四个目的地是否都点得到、52px 折行后内容首行的 y 坐标、以及 `SelectionBar` / `pb-24` 这两条（它们依赖滚动，静态画布看不见）。§6.8 第 38 条的 `iframe` 预览把**折行**这一条做到了可视，但它是 1280/390 两个写死的宽度，不是真机。
...
9. **亮/暗双套截图**。每步完成后各截一组全站图（文件列表/网格、存储、元数据、备份、登录、预览弹窗、右键菜单、浮条、空态、加载态）。**这套截图本身就是验收物**，附在对应 commit 的说明里；没有它，"改好了"只是感觉。
   ...
   **UI-13 给这套方法补了缺的一块：媒体查询状态。** 静态预览只有一个视口，所以 `sm:` 那类断点从来没被触发过；把同一份标记 + 同一份内联样式表放进**固定宽度的 `iframe`**，媒体查询就按那个宽度解析 —— 依旧是真实视口，依旧画不出应用画不出的东西。UI-13 的四格（亮/暗 × 1280/390）就是这么出的，`.workbuddy/preview/shell.html`。
10. **同心算术的守卫（UI-12 新增，计划里没有）**。`src/react-app/geometry.test.ts`：从 `index.css` 读 `--radius`，按阶梯算出每对配角的像素，然后断言**三条**——① `内层 ≤ 外层 − 边框 − 内边距`（偏深一律不允许）；② `(外层 − 边框 − 内边距) − 内层 ≤ 1px`；③ `1px < xs`，即容差必须小于它代偿的那一步。**②的容差在 UI-20 换过依据**：原文写「1px，取自库自己菜单行的 1px」，但库在自己默认旋钮下的菜单其实是 24 / 4 / 16（间隙 4px）——那个 1px 是我们自己 5px 旋钮下的产物，被误当成了库的惯例。现在的依据是它**刻意小于这一档阶梯最小的一步**（`xs` = 0.25 × 旋钮 = 1.5px），所以间隙超过它就意味着**有可用的一步被跳过**，而不是「没有可用的一步」；③ 守的正是这条关系。**配对表同时加了 `border` 一项**，用来算上面那个被漏掉的 1px 描边。**为什么值得加**：§5.1 担心的正是"配错了不会报错"，而 §5.2 的四个测试文件**一个都没红**——类名是稳定的，px 才是会坏的，静态渲染测试永远看不见这一层。当前 3 对：分段控件、右键菜单、浮条。
    **它的边界写在文件头里，别过度信任**：它只覆盖列进去的配对，且当某个组件的 inset 或内层角改动时**这张表要跟着改**——它守的是算术，不是"组件里写的类和表里一致"（那是渲染测试的事）。**三个方向都验证过会红**：把分段控件内层改回 `rounded-md` 的 4.5px → `inner 4.5px against 3px of room`；把菜单行改成 `lg` → `inner 6px leaves 6px of the corner band unused`；**把菜单 inset 改回 `p-1`（旋钮走了、内边距没走）→ `inner 12px leaves 2px of the corner band unused`**——最后这条是 UI-20 加的，它守的是一个此前没人守的错法。**这也说明为什么第二条断言是必要的**：只写第一条的话，"菜单行用 5px" 这种明显漂移会静默通过。
...
| UI-13 | ✅ 完成 | **导航从第五份手写配方回到共用组件。** 四个目的地此前是 `rounded-lg px-3 py-2 text-sm` + `border` 的手写按钮，而三个已抽取的分段控件用填色 + `text-xs` —— 同一个意图，已经漂了；现在走 `SegmentedControl` 的 **`md` 档**（新增）。**顶栏 64 → 52px**（`sm:h-13` → `calc(var(--spacing) * 13)`），`bg-surface/95 backdrop-blur` → **`bg-background/80` + `material-bar`**，**去掉 `border-b border-separator/80`**。**修掉那个硬 bug**：`nav` 原是 `hidden sm:flex`，手机上四个目的地整条不存在、没有任何替代路径；现在 `<sm` **折两行**（第一行品牌 + 右侧控件，第二行导航占满宽度、可横向滚动），用 **`order-last` 而不是移动 DOM**，因为 Tab 顺序在两种宽度下都必须是 品牌 → 导航 → 控件 → 内容，而那正是 DOM 顺序（§6.8 第 32 条）。**内容加 `arrive`**：`key` 取**目的地**而非 `pathname`（取 pathname 会让每次进目录都重挂 `FilesPage`、重取列表、丢掉加载态），`--arrive-from` 就地设 **2px**（键帧默认 6px 是"页面里的消息"的量，整块内容走 6px 读起来像窗口在滑）；复用 `arrive` 本身即满足了 §5.5 的 reduced-motion 兜底。**`size` 档只动选项的高度与字号**，不动外层圆角与内边距，所以同心配对角在两种尺寸下都是 6 − 1 − 2 = 3px，守卫仍只占一行。**新增 `app-shell.test.tsx`（5 条）**：四个目的地都在标记里、`nav` 上没有 `hidden`、导航里确实是共用的分段控件（`role="group"` + `md` 的 `h-7` + 没有旧的 `text-sm`）、阅读顺序、顶栏的材质与无下边线、`arrive` 与 `[--arrive-from:2px]`；`brand-views.test.tsx` 补一条把外层开标签在两种尺寸下钉成逐字节相同（守卫读的是数字表，看不见这一半）。**探针两个方向都验过**：`hidden sm:flex` 加回去 → 第 1 条红；`size` 改成 `sm` → 第 2 条红。**验证**：`prettier` ✓ / `eslint` ✓ / `tsc -b` ✓ / `vitest` **639 passed**（633 + 5 + 1）/ `vite build` ✓（重建后哈希与体积不变，可重复）；23 个新 class 全部按**转义形式**从 `dist/client/assets/index-Jv-DgNdK.css` 里 grep 出来，含 `.\[--arrive-from\:2px\]` 与 `sm:h-13`。**新增无浏览器预览** `.workbuddy/preview/shell.html`：§5.9 的方法加一条新做法——媒体查询状态放进固定宽度的 `iframe` 里才解析得对，见 §6.8 第 37 条。**仍未做**：390px 的真实可达性、Tab 顺序、52px 折行后的首行位置，这三条要浏览器，见 §5.6。 |
| UI-14 | ⏳ 待办 | |
| UI-15 | ⏳ 待办 | |
| UI-16 | ⏳ 待办 | |
| UI-17 | ⏳ 待办 | |
...
| UI-20 | ✅ 完成（用户要求：「让圆角更圆一点，符合 macOS 27 的设计」） | **整体变圆一档，幅度主要给控件。** 旋钮 5px → **6px**：面板 15 → **18px**，菜单/浮条行 10 → 12px，分段控件与预览框 5 → 6px，小盒子 3.75 → 4.5px；`--radius-control` 8 → **10px**；字段从库的 `1.5 × 旋钮`（7.5px）**绑到控件档**（10px）——输入框与按钮从此是同一个值，而不是差 1px。**先查了外边那个数才定**：macOS 26 Tahoe 窗口圆角 26px、macOS 27 Golden Gate 收回 15-16px，所以 UI-11 定的 15px 面板本来就正好压在 macOS 27 的窗口值上，偏方的是控件那一档（8px 在 36px 高的按钮上只有 22%）。连带：菜单与浮条 inset `p-1` → `p-1.5`（面板对只有 `内边距 = --radius` 时才严格同心）；守卫补上被漏掉的 1px 描边项、容差依据换成 `xs` 并加第三个断言；菜单/浮条两条渲染测试补钉 inset。验证：`prettier` ✓ / `eslint` ✓ / `tsc -b` ✓ / `vitest` 633 passed / `vite build` ✓；产物里 `--radius:.375rem`、`--radius-control:.625rem`、`--field-radius:var(--radius-control)`、`.button` 覆盖都在，并**量过层归属**（§6.7 第 28 条）。 |
...
### 6.7 用户要求的第二次几何调整（UI-20）

24. **先查了外边那个数，结论和直觉相反，这一条值得单独记。** macOS 27 Golden Gate 的窗口圆角是 **15-16px**，比 macOS 26 Tahoe 的 **26px** 更**小**（`defaults write -g NSConvolutionOverride1`：10 = macOS 15 Sequoia、16 = Golden Gate、26 = Tahoe）。也就是说 Apple 在 27 上做的是**收紧**——把 Tahoe 那套"半径随窗口内容浮动"的不一致收成一个统一值；用户说"更圆一点、符合 macOS 27"时，这两半其实是互相拉扯的。查完之后的判断是：**UI-11 定的 15px 面板本来就正好压在 macOS 27 的窗口值上**，真正读起来方的是控件那一档（8px 按钮在 36px 高上只有 22%，输入框 7.5px）。所以做法不是把面板推到 Tahoe 的 26px，而是面板抬一级到 18px（比窗口软一档，同一风格族）、**幅度主要给控件**（按钮与输入框 → 10px，25-28%）。这条也可以反过来用：**"符合 macOS N" 这类要求，先去量那一版的真实数值，再决定改哪一档**——直觉说"新版本更圆"，量出来是更方。
25. **面板对的严格同心要求 `内边距 = --radius`，所以 inset 必须跟着旋钮走。** `外层 − 内边距 − 内层 = 3r − b − 2r = r − b`，只有 `b = r` 时为零。UI-11/12 时旋钮 5px、inset 4px（间隙 1px，在容差内）；旋钮到 6px 而 inset 不动就变成间隙 2px，直接越过容差。**这不是调容差的时候，是调 inset 的时候**——inset 不跟着旋钮走，等于把旋钮动的量原样加到角带上。菜单与浮条因此 `p-1` → `p-1.5`（渲染测试同时补钉了 inset，因为守卫读的是表、不是组件）。
26. **顺带补了一个一直漏掉的项：外层盒子的 `border`。** 分段控件外层是 `rounded-lg border border-border p-0.5`，1px 描边画在盒子内侧，和 padding 一样吃掉角上那一像素，而守卫此前的算式里只有 `外层 − 内边距`。加上它之后，那一对在 6px 旋钮下正好严格同心（6 − 1 − 2 = 3 = `rounded-[3px]`），class 不用改；**改的是账本**。配对表因此多了一个 `border` 项。
27. **容差的依据换掉了，因为它原来是错的。** 原文写"1px，取自库自己菜单行的 1px"——但库在**自己默认旋钮**下的菜单是 24 / 4 / 16，间隙是 **4px**；那个 1px 是我们自己 5px 旋钮下的产物，被我当成了库的惯例。现在 1px 的理由是**它刻意小于这一档阶梯最小的一步**（`xs` = 0.25 × 旋钮 = 1.5px），所以间隙超过它意味着**有可用的一步被跳过**，而不是"没有可用的一步"；这条关系本身加了断言 ③（`1px < xs`）守着，免得以后旋钮变小、容差反而变得比阶梯还粗。
28. **`@theme` 是覆盖库已有 token 的错误位置——同层、更靠后，会输。** 把 `--field-radius` 绑到 `--radius-control` 时先写在了 `@theme` 里，构建产物一量才发现**根本没生效**：`@theme` 的输出落在 Tailwind 的 `theme` 层，而库的 `--field-radius: calc(var(--radius) * 1.5)` 也在 `theme` 层、位置更靠后（`@import "@heroui/styles"` 展开在 Tailwind 自己的 theme 块之后），同层同特异度 → 后者胜。移到文件尾部的**无层** `:root` 才生效。反过来，库的 `--radius` 输给本仓，原因正是它分层而本文件不分层（无层恒胜，与源序无关）。**量法**：按括号深度遍历产物的顶层块，把每个字节位置归到所属的规则与层——我们的 `--field-radius` 在 4866 字节 `layer=theme`、库的在 5626 同一层；改完之后我们在 426491 无层。**结论留成规律**：新 token 放 `@theme` 没问题（没有冲突对象，`--radius-control`、`--color-hairline` 都在那里），**切库已有的 token 必须无层**。
29. **A/B 预览：把旧 token 值重新声明在一个容器上。** §5.9 那套静态预览这次用来做前后对照——同一份标记（真实组件渲染的）放四个容器：亮/暗 × 前/后，其中"前"的容器只多做一件事：

    ```css
    .before { --radius: 0.3125rem; --radius-control: 0.5rem; --field-radius: 7.5px; }
    ```

    `.before` 与 `:root` / `[data-theme]` 同为 (0,1,0) 特异度、位置在后，所以恒胜。这样两侧是**同一份标记、同一份生产样式表，差别只有旋钮**，比画一张示意图可信得多。边界：它只能对照 token 能表达的东西——菜单 inset 从 `p-1` 到 `p-1.5` 是 class 变更，两侧都会显示新值，所以预览里看不出这一处（要靠几何守卫）。生成器仍然是临时的、用完即删。
30. **生成器踩了一个坑，值得记：`dist/client/assets` 里有四个 `index-*.css`。** 除了 Tailwind 那一份（435KB），还有三个是 Monaco 的 chunk 拉进来的（1KB / 1KB / 30KB，其中两个就叫 `index-*.css`）。按 mtime 取"最新"会随机拿到 Monaco 的，页面就变成没有样式的裸 HTML（第一次生成的 28KB 文件就是这么来的）；**按体积取最大**才对。上一轮做登录页预览时用的是写死的文件名，换成通配就得注意这件事。

### 6.8 第四次修正（UI-13 执行中发现，外壳）

31. **"分段控件外层换填色"这一步属于 UI-15，不属于 UI-13 —— R5 与 §2 在这里打架，按 §2 走。** R5 的导航条目写的是"容器把 `border border-border` 换成 `bg-background-tertiary` 填色"，而 §2 把"12 处边框改填色"整体放在 UI-15，文件清单里就列着 `SegmentedControl.tsx`。**不按 R5 走的理由不是"计划写了"，而是两条具体代价**：① `SegmentedControl` 是**共用**组件，在"外壳"这一步换掉它的外框，会顺手改掉 `Pager` / `MarkdownViewer` / `FileToolbar` 三处的观感——那三处既不在这一步的文件清单里，也没被这一步的任何判据覆盖；② 外框一去掉，守卫那一对的 `border` 就从 `1` 变 `0`、内层要从 `rounded-[3px]` 变成 4px，`brand-views.test.tsx` 的同心断言也要跟着改。**那是 UI-15 的账**，所以这一步**保留 1px 描边**，填色与其余 11 处一起在 UI-15 做。执行 R5 时按此理解：R5 描述的是这一步**结束时**的样子，不是它该在哪儿做。
32. **手机上的两行折叠用 `order-last`，不是把 `nav` 挪到 DOM 后面。** R5 只说了"折两行"，没说怎么折。两种写法在**视觉上完全一样**，差别在 Tab 顺序：本步的判据是 **品牌 → 导航 → 右侧控件 → 内容**，而这正是 DOM 顺序（`nav` 在品牌与控件之间），所以折行只能靠 CSS 的 `order` —— 把 `nav` 挪到 DOM 末尾会让 Tab 顺序变成 品牌 → 控件 → 导航 → 内容，判据直接不成立。代价要写下来：**手机上的视觉顺序（品牌、控件、导航）与 Tab 顺序（品牌、导航、控件）不一致**，焦点会从视觉上跳到上面一格。这是判据要的那个权衡，不是疏忽；将来有人想"修掉"这个不一致，**要改的是判据，不是 `order`**。
33. **`NAV_ITEMS` 必须从数组改成按 kind 建索引的记录。** 原来是 `{kind, label, path}[]`，因为旧渲染把三个字段都读了一遍；**分段控件只会回报被选中的那个值**，路径得再查一次，而 `find(...)?.path ?? 回退` 这种写法等于承认"可能查不到"。现在是 `NAV_ORDER`（元组，同时给出 kind 的联合类型与显示顺序）+ `NAV_ITEMS: Record<NavKind, { label, path }>`，由编译器保证两者不脱节 —— 数组则允许"顺序表里有它、条目里没有"这种半成品状态。顺带 `AppRoute` 这个 import 不再需要。
34. **`size` 档绝不能动外层的 inset，这一点由断言守着。** 同心配对在守卫里是**一行**（外层 `lg` / inset `0.5` / border `1` / 内层 `3px`）。如果 `md` 顺手把 padding 也改了，这一行就只描述 `sm`，而**没有任何测试会说话** —— 守卫读的是数字表，渲染测试只看得见类名，正是 §5.1 担心的那类静默失效。所以补了一条断言：两种尺寸下**外层开标签逐字节相同**（取第一个 `>` 之前的全部，因为没有任何属性里含 `>`），变的只能是选项。规律：**`size` 只属于选项，容器几何与尺寸无关**；`geometry.test.ts` 的配对表注释也补了这句。
35. **顶栏的分离度在 UI-14/15 之前是变*薄*的 —— 这是中间状态，不是回归，必须写下来。** R2 说顶栏靠"96.4% 的玻璃压在一个高一档的内容平面（`bg-surface`）上"分开，而**那个平面是 UI-14/15 才出现的**。所以在这两步之前，顶栏与页面同为 `--background`，静止时唯一的差别只剩材质与 20% 的透过率 —— **比改动前（有一条下边线）更弱**，只有内容滚到它下面时才看得出来。计划就是这么排的（UI-13 去掉线、UI-14/15 造平面），但不写下来，第一个看到的人一定会以为改坏了。预览（`.workbuddy/preview/shell.html`）里也是这个样子。
36. **内容入场动画的 `key` 必须是目的地，不能是 `pathname`。** 直觉写法是 `key={pathname}`，那样**每次进目录都会重挂 `FilesPage`**：重新取列表、丢掉加载态，一个纯为动效引入的回归（`FilesPage` 是 `*` 路由，同一个组件实例承载所有目录）。按 `route.kind` 取键之后，目录内导航键不变、不重挂，换目的地才动 —— 而那时子树本来就整个换了，所以重挂成本为零。
37. **`--arrive-from` 在这个元素上是 2px，不是键帧默认的 6px。** 用任意属性类 `[--arrive-from:2px]` 就地覆盖。理由与 R8 把"控件 200ms / 浮层 300ms"分开是同一条：**整块内容位移 6px 读起来像窗口在滑动**，2px 才是页面过渡的量；6px 那个默认值是给"页面里到达的一条消息"的。**复用 `arrive` 而不是新写一个动画**，正是因为它自带 `prefers-reduced-motion: reduce { animation: none }` —— §5.5 那条要求靠复用满足，没有新增第二处需要记得加兜底的动效。核过产物：`.\[--arrive-from\:2px\]{--arrive-from:2px}` 与键帧里的 `var(--arrive-from,6px)`，所以取到的是 2px。
38. **媒体查询状态要用固定宽度的 `iframe` 才预览得出来 —— 这是 §5.9 那套方法缺的一块。** §5.9 的静态预览只有一个视口，所以 `sm:` 这类断点**从来没被触发过**；而 UI-13 恰好**就是**一个断点改动，"390px 下折两行"这条判据在单文档预览里根本画不出来，只能靠断言。做法：把**同一份标记 + 同一份内联样式表**放进 `<iframe width="390">` —— 那是**一个真实视口**，媒体查询按 390 解析，不是伪造出来的状态，也没有违反"预览不可能显示应用渲染不出来的东西"。这一次放了四格（亮/暗 × 1280/390）；`srcdoc` 需要把 `& < > "` 四个字符转义（样式表里有 `>` 和 `"`），代价是体积约 1.8MB。生成器照旧按体积取最大的 `index-*.css`（§6.7 第 30 条），四格里内联的那份 CSS 都与构建产物**逐字节相同**。**边界照旧**：仍然只有静态画布，hover / focus / 菜单展开 / 错误条都不在里面；文件是临时产物，方法记在这里而不是留在仓库里。
39. **同一个文件不要并行发多个编辑 —— 会丢一次，而且症状是"文件看起来改了一半"。** 给 `SegmentedControl` 加 `size` 时，把"补注释"和"加类型 / 签名"两条编辑放在**同一条消息**里，工具对两条都回成功，但只有一条落盘：另一条基于**编辑前**的快照写成，被覆盖掉了。看起来像"改了一半"，实际报错要到运行时才出现（`ReferenceError: OPTION_SIZE is not defined`），而文件里 `OPTION_SIZE[size]` 明明写着。规则：**一个文件一次只发一条编辑**，多处改动串行发。同一轮里 `App.tsx`、`brand-views.test.tsx`、`i18n.ts` 的多次改动因为都是串行的，没出过这个问题。
40. **工作区里的"删除"不是删除：`rm` 会把目标移进仓库根的 `.Trash-0/`，而 `.Trash-0` 不在 `eslint.config.js` 的 `ignores` 里。** 本轮删临时生成器时踩到：`rm` 之后仓库根冒出一个未跟踪的 `.Trash-0/`，里面装着刚"删掉"的那个文件；再对 `.Trash-0/` 里的路径执行 `rm`，只会**再套一层**（`.2` 后缀、`.trashinfo.trashinfo`），删不干净。**清它要用程序化删除**（`shutil.rmtree`），`rm` 不行。这条在 UI-12 那轮已经因为 `.Trash-0` 里的 `dist` 产物把 `eslint .` 撑爆过一次（见长期笔记），现在补上"为什么删不掉"这一半。
