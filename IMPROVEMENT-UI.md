...
1. **同心圆角逐处复核**。全仓 30+ 处圆角一起变小，配错的地方会"看起来只是有点怪"，不会报错。规则化：`内层 = 外层 − 内边距`。必须逐条过的配对：面板卡↔行、分段控件↔选中段（**顶栏导航也走这一个，UI-13 新增**）、菜单↔菜单项、浮条↔动作按钮、`Modal`↔`Body`、字段↔内嵌小元素。**建议在 `index.css` 里为每处配对留一行注释说明为什么是这个数**（上一轮 UI-10 的 `SegmentedControl` 注释是好的先例）。
2. **测试里的实现细节断言**。本轮会改到 4 个文件的断言：
   - `ContextMenu.test.tsx:92-93`（`rounded-3xl` / `rounded-2xl`）
   - `file-views.test.tsx:399-400`（同）
   - `brand-views.test.tsx:45-46`（`rounded-lg` / `rounded-md`）
   - `preview-views.test.tsx:132`（`rounded-lg border border-border`）

   这些断言本来就在测实现细节，几何一变就红。**处理原则**：迁移到语义断言（`data-open`、`role`、`data-icon`、`aria-*`）优先；确实需要断言几何的（同心配对）保留但换成新数值。**迁移必须在同一步内完成**，不允许"先删断言让测试变绿、样式随后再改"。
...
8. **`.button` 覆盖生效**。`index.css` 是无层样式，理论上恒胜于 Tailwind 的分层输出，但**必须实测**：渲染一个 HeroUI `Button`，确认 `border-radius` 是 8px 而不是 15px（DevTools 里看 computed style，不要只看产物里有这条规则）。
