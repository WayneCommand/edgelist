...
| **UI-18** | 表单分组（R10）+ 登录页 + 收尾：`IMPROVEMENT-UI.md` §6.4 执行记录 | `BackupPage.tsx`、`StorageForm.tsx`、`StorageFields.tsx`、`MetadataPage.tsx`、`LoginPage.tsx`、本文档 | 全量 `prettier --check src` → `eslint .` → `tsc -b` → `vitest run` → `vite build` 全过；`vite build` 产物里确认新 token 与 `@utility` 都生成 |
...
1. **同心圆角逐处复核**。全仓 30+ 处圆角一起变小，配错的地方会"看起来只是有点怪"，不会报错。**规则在 UI-12 收窄成单向的**（原文写的是"内层 = 外层 − 内边距"，那是把一件事写成了等式，而库自己就不满足它——它自己的菜单是 24 / 4 / 16）：

   > **内层 ≤ 外层 − 内边距**。内层偏小只是"角略紧"，看不出来；内层偏大才会切进外圈的弧。

   按这条逐处过：面板卡↔行、分段控件↔选中段（**顶栏导航也走这一个，UI-13 新增**）、菜单↔菜单项、浮条↔动作按钮、`Modal`↔`Body`、字段↔内嵌小元素。**UI-12 走完的结果**：只有 `SegmentedControl` 真的违反（5 − 2 = 3，而 `rounded-md` 是 3.75px），已修；菜单/浮条那一对是 15 / 4 / 10，比 11 小，属于"角略紧"，保留（并与库的 `.menu-item` 一致）。**每处配对在代码旁留一行注释说明为什么是这个数**——UI-12 已给 `SegmentedControl`、`ContextMenu`、`SelectionBar`、`metrics.ts`、`StorageTable` 都补上了，并把 `--radius-control` 的三条"不适用"情形写在 `index.css` 的 token 注释里（见 §6.5）。算术本身有自动化守卫，见 §5.10。
...
10. **同心算术的守卫（UI-12 新增，计划里没有）**。`src/react-app/geometry.test.ts`：从 `index.css` 读 `--radius`，按阶梯算出每对配角的像素，断言 `内层 ≤ 外层 − 内边距`。**为什么值得加**：§5.1 担心的正是"配错了不会报错"，而 §5.2 的四个测试文件**一个都没红**——类名是稳定的，px 才是会坏的，静态渲染测试永远看不见这一层。这个文件把坏的那一层拿出来单独守（当前 3 对：分段控件、右键菜单、浮条）。
    **它的边界写在文件头里，别过度信任**：它只覆盖列进去的配对，且当某个组件的 inset 或内层角改动时**这张表要跟着改**——它守的是算术，不是"组件里写的类和表里一致"（那是渲染测试的事）。已验证它是会红的：把分段控件那行改回 `rounded-md`，它会以 `inner 3.75px in 3px (outer 5px less 2px)` 失败。
...
12. **同心规则从等式收窄成不等式：内层 ≤ 外层 − 内边距。** 原文写的"内层 = 外层 − 内边距"是错的，**库自己就不满足**——它的菜单原本是 24 / 4 / 16，16 ≠ 20，即内层比等式要求的**更小**。而更小无害（角略紧，看不出来），更大才有害（切进外圈弧）。按等式去"修"菜单会把它从 10px 改成 11px，反而偏离库。**这条收窄是这一步最有价值的产出**：它把"30 处都要重算"变成"只有 1 处真的坏了"，而且解释了为什么只有那一处——分段控件是唯一一处**当初按等式配的**。§5.1 已改写。
13. **裸 `rounded` 是 Tailwind 的固定 `.25rem`，不跟随 `--radius`。** 产物里它是 `.rounded{border-radius:.25rem}`，与阶梯无关。这是"全仓无几何字面量"这条判据里最隐蔽的一类：**它看起来像阶梯成员，实际是内置常量**，所以降旋钮时它不会跟着动，而代码评审时也没人会觉得一个裸 `rounded` 可疑。清掉 6 处（`FileListSkeleton`、`FileTable` 表头、`DirectoryTree` ×2、`StorageTable` 的 `ACTION_CLASS` 与 3 个骨架）。**留下的教训**：查几何字面量不能只查 `[Npx]`，要连裸工具类一起查。
14. **新增了一条计划外的守卫（§5.10）**，放在 `src/react-app/geometry.test.ts`。理由见 §5.10，简短版：§5.1 担心的"配错了不会报错"，而 §5.2 又证明渲染测试看不见 px——那么**算术就是唯一还能自动守的东西**。它从 `index.css` 读 `--radius`（而不是抄一份 token 值），按阶梯算 px，断言不等式。**已验证它会红**（把分段控件那行改回 `rounded-md` → `inner 3.75px in 3px (outer 5px less 2px)`）。边界写在文件头：只覆盖列出的配对，改配对时要改表。
15. **`--radius-control` 的适用范围是靠实做才说清的。** UI-11 只给了"按钮专用的一档"这句话，落地时立刻撞上三类边界，全部写进了 `index.css` 的 token 注释：①**面板内的控件**（右键菜单行、浮条动作）跟面板走（`rounded-2xl`）——因为同一个动作在两处出现，Menu 里 10px、浮条里 8px 就是"同一个按钮换了形状"；②**行内小按钮**（面包屑铅笔、表头排序、存储行动作，20-26px 高、hover 才出现盒子）取小阶梯步——8px 在 20px 的盒子上是个鹅卵石，而且存储行里它们旁边就有一个 `rounded-md` 的 driver chip，两个小盒子同排不同角才是真正的漂移；③**chip 是标签**，留在 `rounded-full`。同时发现一个副作用并写进注释：因为 `.button` 覆盖是无层的，`<HeroButton className="rounded-control">` **不会生效**（覆盖恒胜于工具类）——不需要它，但要知道。
16. **§3.10（不动 `metrics.ts`）与"无几何字面量"冲突了一处。** `MarkdownViewer` 和 `preview/index.tsx` 的 Suspense 骨架各自写了一遍 frame 的圆角，与 `metrics.ts` 的 `FRAME` 是同一个值的三份拷贝。**做法是提取而非改值**：新增 `FRAME_RADIUS`，`FRAME` 由它组合，两个骨架改用它——`FRAME` 拼出来的串与原来逐字符相同，所以 `preview-views.test.tsx` 的断言也不需要改）。这样"全仓除 `metrics.ts` 外无几何字面量"这句话才成立。
17. **顺手升级了一种检查方法：产物 CSS 的层结构可以直接量出来。** 写一个按括号深度遍历顶层块的脚本，就能回答"我这条规则落在哪一层、前面还有哪些层"。UI-11 的 §5.8 因此从"结构推断"变成"实测"（`utilities` 层在 454785 闭合，项目规则在 458k 之后的无层区，中间没有层可输）。**没有浏览器时这是把"应该生效"变成"必然生效"的唯一办法**，值得留给后面几步。
