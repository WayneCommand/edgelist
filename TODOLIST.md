...
- [x] 8.6b 前端分片上传接入上面五个端点；非 S3 驱动时降级并提示上限。
  - **上限取 100 MB，而且只有这一个数字站得住**：Cloudflare 按**账号套餐**限制请求体（Free/Pro 100 MB、Business 200 MB、
    Enterprise 自助最高 5 GB），超限的请求在**边缘就被 413 挡掉，Worker 代码根本不会运行**——所以服务端给不出更友好的提示，
    客户端也问不到自己是什么套餐。100 MB 是文档化的**下限**，Business/Enterprise 部署只是比理论必要值更早走分片，
    代价是几次额外往返，没有别的。
  - **能力从 worker 流到前端**：`getDriverInfo()` 增加 `capabilities`（`Set` 过不了 `JSON.stringify`，排序让载荷稳定）。
    前端 `chunkableMounts()` 由「存储的 `driver`」+「驱动的 `capabilities`」推出哪些挂载点能分片——用的是 worker 解析时
    用的**同一份注册表**，所以两边的答案不可能走偏。
  - **`chunkable` 是三值的**：`true`/`false` 是答案，`null` 是「还不知道」（存储表或驱动表没加载出来）。未知时**照分片试**，
    理由和 `unwritableHint` 一样：猜「不能」会拒掉一个本来能成的上传；猜「能」只多一个请求，而 worker 会用清楚的话拒掉它，
    何况超限文件本来也没有别的路可走。
  - **传输按文件大小逐个选**：`≤100 MB` 走单个 `PUT /api/fs/put`，`>100 MB` 走分片，非对象存储**在字节移动之前**就拒掉
    并说明上限。
  - **分片大小不听前端**：用 `init` 返回的 `chunk_size`。5 MiB 的下限（S3 除最后一块外不接受更小的分片）和默认值都归
    服务端所有，前端再存一份就是一份会漂移的副本。
  - **块一个一个发**：会话在 KV 里，每块都要读-改-写回，两个并发就会互相覆盖，输的那块的 part number 会从记录里丢掉，
    而 provider 那边其实留着它（`multipart.ts` 顶部已注明）。
  - **任何失败都 abort**：Worker 既没有常驻内存也没有后台任务，会话留着就等于把 provider 侧的分片一直挂到过期。
    边界分得很清——`init` 失败**不** abort（会话根本没开，abort 一个不存在的会话 id 是无意义的请求），
    块失败或 `complete` 失败才 abort（尽力而为，会话可能已经没了）。
  - **进度要动**：分片文件自己报字节百分比（`UploadProgress.bytes`），否则「上传 2/5」会几分钟不动，读起来像卡死。
  - **已知取舍（记在这里而不是悄悄留着）**：`multipartInit` 不发送 `Content-Type`，所以走分片上传的对象会拿到 provider 的
    默认类型，而单请求 `PUT` 会带上真实类型。预览侧的 `withMime()` 按扩展名兜住了，所以这一步没有扩到适配器接口里去改——
    要改就是给 `multipartInit` 加参数，属于 8.6a 已定接口的扩张。
- [x] 8.7 前端上传体积上限提示与失败重试。
  - **重试的前提是能分清「拒绝」和「抖了一下」**：`api()` 现在抛 `ApiError`（带 `status`）。403/404/413 再问一百遍也是
    同一个答案，429/5xx 才是「稍后再来」；`fetch` 没到服务器时抛的是 `TypeError`，没有 status 可查，算可重试。
    `ApiError` 继承 `Error`，所以既有那些 `instanceof Error` 的调用点一行都不用改。
  - **只重试块**（`RETRY_DELAYS_MS = [500, 1500]`，共 3 次尝试）。块是这里**唯一可安全重复**的步骤：服务端按 part number
    替换而不是追加，所以重传不会产生第二个分片。`init`/`complete` 不重试——第二次 `init` 会留下一个没人认领的会话，
    第二次 `complete` 会发现会话已经被第一次删掉了。
  - **单个 PUT 也重试**：大 body 是这个应用里最容易撞上断连的东西，而 `PUT` 到某个路径是幂等的（同样的字节落到同一个
    位置），代价是重发一次 body——仍然比丢掉它好。
  - **上限提示落在会咬人的地方**：只有确定不能分片的存储才在工具栏显示一句能力说明（`data-testid="ceiling-hint"`），
    **未知不提示**——对一个可能并不存在的限制报警，会让人连真的警告一起无视。目录压根没有存储时 `writeHint` 已经说了，
    不重复第二遍。
...
>
> **8.6b / 8.7 验证**：新增 `lib/upload.test.ts`（31 例：上限边界与三值 `chunkable` 的路由、上限文案、
> 上限提示只在确定不能分片时出现、分片数与服务端 `Math.ceil(size/chunk_size)` 一致、切片首尾相接无缝无重叠、
> 非法尺寸返回空、百分比夹取、能力推导（只有 `multipart` 驱动、挂载点写法归一、驱动键不分大小写、没有 capabilities
> 视为没有）、注入式传输的完整循环（顺序、块大小来自会话、累计进度、块失败/拼装失败/init 失败三种 abort 边界、
> 零字节、瞬时失败重试一次、重试耗尽后放弃、拒绝不重试）与 `uploadFile` 的三种情形（单请求、502 后重试、403 不重试））
> 与 `lib/retry.test.ts`（10 例）；`registry.test.ts` 新增 1 例守卫（`capabilities` 必须与驱动自己的声明一致，
> 且 `object` 含 `multipart`、`webdav` 不含），`file-views.test.tsx` 新增 3 例（字节百分比、无上限提示、有上限提示
> 且按钮不被禁用）。合计 **523 例 / 33 文件**；`eslint .` / `prettier --check src` / `tsc -p tsconfig.app.json` /
> `tsc -p tsconfig.worker.json` 全绿。
> **写这一步时改掉一个测试自身的错误**：「重试耗尽后放弃」那条第一次是用 `failOnceChunk` 写的，而它在重试那次**会成功**，
> 于是断言"应当失败"的用例反而通过了上传——换成 `failTransientChunk`（每次都失败）才真的走到耗尽分支。
> 这说明假实现比真代码更容易写错，所以每条重试用例都同时断言 `chunk`/`complete`/`abort` 三个记账数组。
>
> **未做**：这一步仍然**没有真浏览器验收**，理由与 8.6a 相同——分片上传要真的往桶里写数据，而本机的 IBM COS 是
> 用户的生产桶。要验的话需要挑一个可丢弃的前缀（例如 `/waynecos/tmp-upload/`），用真浏览器上传一个 >100 MB 的文件，
> 断言：`init` → 若干个 `chunk`（块大小 8 MiB）→ `complete`，且列表中随后出现该文件；中途断网应看到块级重试；
> 对 WebDAV 挂载点选一个大文件应当**在发出任何字节之前**就被拒并显示上限。
...
- [x] 三个管理页的 i18n（存储 / 元数据 / 备份）。机制已就绪，只需往 `EN` / `ZH` 加分组并把字符串换成 `t(...)`。
  - 新增 `manage.eyebrow` 与 `storages.*`（45 条）/ `meta.*`（25 条）/ `backup.*`（12 条）四组，`action.edit` 补进共用动作组；
    8 个文件换成 `t(...)`：三个页面加 `StorageTable` / `StorageFields` / `StorageForm` / `StorageJsonDialog` / `DriverField`。
