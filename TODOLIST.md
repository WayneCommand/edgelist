...
- [ ] 1. 建立项目 TODO、提交规范和验收清单。
- [ ] 2. 对照 OpenList 参考代码梳理 API、数据模型、备份格式和认证行为。
- [ ] 3. 升级并固定项目基础配置，接入 Tailwind CSS 与 pnpm 工作流。
- [ ] 4. 建立 Worker 的配置类型、KV 绑定和统一错误/响应模型。
- [ ] 5. 实现 AK/SK 登录、KV 校验、会话令牌和鉴权中间件。
- [ ] 6. 实现 OpenList 兼容的存储配置模型与存储适配器接口。
- [ ] 7. 实现 OpenList 存储适配器。
- [ ] 8. 实现对象存储适配器。
- [ ] 9. 实现 WebDAV 存储适配器。
- [ ] 10. 实现文件/目录元信息 API（列表、详情、创建目录、删除、重命名）。
- [ ] 11. 实现文件传输 API（上传、下载、直链/重定向及必要的分片行为）。
- [ ] 12. 实现 OpenList 兼容的备份导出格式。
- [ ] 13. 实现 OpenList 兼容的备份导入与还原校验。
- [ ] 14. 用 React + Tailwind 重建 OpenList 登录界面。
- [ ] 15. 实现 OpenList 风格的文件浏览、搜索、操作和存储管理界面。
- [ ] 16. 实现前端登录态、鉴权失败处理和备份/还原界面。
- [ ] 17. 增加 API/适配器/备份还原测试与 OpenList 兼容性样例。
